...
- `src/index.ts` — additionally export:
  ...
  - A named evaluator type, e.g. `type GateEvaluator<TIdentity, TValue> = (overrideIdentity?: TIdentity) => Promise<TValue>` in `src/lib/types.ts`, used by `GateFactory` instead of inline function types. `createReactGate` accepts the evaluator's plain call signature rather than requiring its full object shape, so bare async functions remain valid after plan 07 adds `details` to `GateEvaluator`.
