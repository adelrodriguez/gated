...
snapshot.details(theme) // EvaluationDetails<"light" | "dark">, sync
```
...
- Initially require unique flag keys within one snapshot and validate that invariant before identity resolution, hooks, or provider work. Throw a descriptive runtime error for duplicates. This avoids ambiguous defaults and variants.
- Dispatch each micro-batch as soon as its leaders finish preparation. It must not wait for every flag, because a dedupe follower can depend on a leader in another concurrent snapshot.
- `snapshot.get` and `snapshot.details` are synchronous and only accept flags that were in the snapshot (type the snapshot over the tuple of evaluators; either method throws for a missing flag).
- Missing key in `decideMany` response → that flag falls back to its single `decide`, then default. Extra keys ignored.
- Anonymous snapshots pass `null` to `decideMany`; cache and dedupe hooks bypass those evaluations exactly as they do on the single-gate path.
...
- `snapshot.details` preserves source, default errors, and variant payloads with typed values.
- Concurrent snapshots with cross-key dedupe leaders do not hang when hook latencies differ.
- Anonymous snapshots pass null identity to each batch and are not deduplicated.
- Snapshot + `dedupeHook`/`cacheHook` integration: cache populated per flag.
