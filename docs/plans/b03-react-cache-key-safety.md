...
- Circular cache-key object throws.
- Distinctness properties (table-driven, property-style): for a corpus of structurally distinct valid keys — `0` vs `-0` vs `"0"`, `null` vs `undefined` vs `"null"`, `{a: undefined}` vs `{}`, `["a"]` vs `"a"`, nested records with reordered keys (equal) vs different values (distinct) — assert serialized equality iff structural equality.
