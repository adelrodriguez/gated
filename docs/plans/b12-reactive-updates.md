...
3. React integration: `createReactGate` accepts `changes` (the hub). Implementation: keep the promise cache, add a per-key version counter bumped by notifications; the hook subscribes via `useSyncExternalStore` to the version, and a version bump both evicts the cache entry and triggers re-render → re-evaluate → suspend/transition. Notifications for keys the component does not use cause no work (version store is per cache key).

   The evaluator→flag-key seam needs designing, not just an accessor: `definitions` is declared inside the `buildGate` closure (src/core.ts:104), so it is unreachable from `react.tsx`, and `createReactGate` must work for evaluators produced by _different_ factories. Attach the flag key to the evaluator itself under a module-private symbol exported from an internal module, and have the React layer read it from there; a hoisted module-scope `WeakMap` in the factory module is the equivalent alternative. Either way it is one internal seam, not a per-factory registry lookup.

   The version store lives in `src/integrations/react.tsx` for this slice. Plan 05 recorded the intended end state — cache plus `subscribe` extracted into a framework-agnostic module (zustand's vanilla/react split) so `react.tsx` stays a thin subscriber and future integrations reuse the store. That extraction is **deferred, not rejected**: it pays off with a second integration, and doing it here would couple a behavioral change to a file move. Revisit when a non-React integration lands.
