...
- `cacheKey` is the explicit seam for functions whose call arguments contain operational, non-semantic values. It receives the normalized arguments and projects the JSON-serializable value that identifies an evaluation. This slice defaults to the full tuple because the current evaluator has only identity arguments. Plan 08 changes the `GateEvaluator` overload's internal default to `(options) => options?.identity ?? null` and requires custom bare async functions to supply `cacheKey`; consumers wrapping Gated evaluators do not configure it at each call site. Plan 09 can then add `signal` without changing cache identity or redesigning this binding.
