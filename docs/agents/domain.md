...
| **recipe key**          | The collision-safe string that a cache or dedupe recipe uses to identify one flag and identity evaluation. It uses the flag key and typed `distinctId` by default and can use a consumer projection.                     |
| **request coalescing**  | Core behavior that lets concurrent evaluations with one flag, compatible gate shape, and identity share provider work after resolve hooks. Each evaluation keeps its own error, fallback, and cancellation path.         |
