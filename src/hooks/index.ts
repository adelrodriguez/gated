 ...
 * mutable state in its closure is shared too. Prefer the factory overload when the hook keeps
 * state.
 */
...
/**
 * Defines a hook factory with typed options. Each invocation produces a fresh hook, so closure
 * state stays isolated per registration.
 */
