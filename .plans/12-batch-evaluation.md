...
Delivers: API opportunity #7. Depends on: 03 (lifecycle), 08 (options object). Additive, non-breaking.
...
- `gate.snapshot(flags, options?)`: resolves identity once, calls `decideMany` once (falls back to parallel `decide` calls when `decideMany` is absent), then runs each flag's normal lifecycle against the pre-fetched decision (resolve hooks may still short-circuit per flag; validation and after hooks run per flag; per-flag failure → that flag's default).
- Implementation route: model the pre-fetched batch as an internal resolve-stage source feeding plan 03's evaluation record — do NOT build a second evaluation pipeline.
...
- `decideMany` called exactly once for N flags; `decide` not called.
